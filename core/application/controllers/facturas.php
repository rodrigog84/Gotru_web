...
public function cargacontribuyentes(){
	...
	public function getAll(){
        ...
        if($opcion == "Rut"){
		  ...
		  foreach ($query->result() as $row)
			...
			left join vendedores v on (acc.id_vendedor = v.id)
			left join tipo_documento td on (acc.tipo_documento = td.id)
			WHERE acc.num_factura = "'.$nombres.'" AND acc.tipo_documento in ( '.$tipo.','.$tipo2.','.$tipo3.','.$tipo4.','.$tipo5.','.$tipo6.')
			order by acc.id desc');
	...
	public function save(){
		...
		$formadepago = $this->input->post('idcondventa');
		...
		foreach($items as $v){
			...
			//$total = $total + (($v->total - $v->dcto) + $v->iva);
		...
		if($tipodocumento == 101 || $tipodocumento == 103){  // SI ES FACTURA ELECTRONICA O FACTURA EXENTA ELECTRONICA
			...
			foreach ($detalle_factura as $detalle) {
				$lista_detalle[$i]['NmbItem'] = substr(trim($detalle->nombre),0,80);
				...
				if($detalle->descuento != 0){
					...
					$lista_detalle[$i]['DescuentoMonto'] = round($detalle->descuento,0);		
	...
	*/
      ...
      $html .='
		...
		*/
		...
		<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
		<title>Libro de Ventas General</title>
		<style type="text/css">
		td {
			font-size: 16px;
		}
