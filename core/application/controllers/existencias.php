...
class Existencias extends CI_Controller {
	...
	public function getAll(){
		...
		if($nombres){
			...
			$query = $this->db->query('SELECT acc.*, c.nombre as nom_producto, b.nombre as nom_bodega, b.id as id_bodega FROM existencia acc
			left join productos c on (acc.id_producto = c.id)
			left join bodegas b on (c.id_bodega = b.id) 
