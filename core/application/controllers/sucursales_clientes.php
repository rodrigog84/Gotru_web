...
class Sucursales_clientes extends CI_Controller {
	...
	public function elimina(){
	    ...
	    $query = $this->db->query('SELECT * FROM factura_clientes WHERE id_sucursal ="'.$idcliente.'"');
	    ...
	    $query = $this->db->query('DELETE FROM clientes_sucursales WHERE id = "'.$idcliente.'"');
	...
	public function getAll(){
		...
		if($nombres){
			...
			cli.rut as rut, com.nombre as nombre_comuna, cli.nombres as nombres,
			cli.id as id_cliente2 FROM clientes_sucursales acc
			left join ciudad c on (acc.id_ciudad = c.id)
			left join comuna com on (acc.id_comuna = com.id)
			left join clientes cli on (acc.rut = cli.rut)
