...
class AdminServicesExcel extends CI_Controller {
    ...
    public function exportarExcelPedidos(){
          ...
          WHERE acc.id_tipopedido = ('.$tipo.') AND acc.fecha_doc between "'.$fecha3.'"  AND "'.$fecha4.'"');
    ...
    public function exportarTXT(){
      ...
      public function exportarExcellistaProductos()
        ...
        public function exportarExcellistaProductos2()
         {
            header("Content-type: application/vnd.ms-excel"); 
            header("Content-disposition: attachment; filename=listaproductos.xls"); 
            ...
            $query2 = $this->db->query('SELECT * FROM precios WHERE id = "'.$id.'"');

            foreach ($query2->result() as $row)
            {
              $nombrelista = $row->nombre;
              $numerolista = $row->numero;
              $margen = $row->margen;
            };

             $query = $this->db->query('SELECT acc.*, c.nombre as nombre, c.codigo as codigo, m.nombre as nom_medida FROM detalle_lista_precios acc
              left join productos c on (acc.id_producto = c.id)
              left join mae_medida m on (acc.id_medida = m.id)
              WHERE acc.id_lista = "'.$id.'"');

            $users = $query->result_array();
            
            echo '<table>';
            echo "<td></td>";
            echo "<td>LISTADO PRECIOS PRODUCTOS</td>";            
            echo "<td>".$nombrelista."</td>";
            echo "<tr>";
            echo "<td>NUMERO: ".$numerolista."</td>";
            echo "<td>MARGEN : ".$margen."</td>";
            echo "<tr>";
            echo "<td>CODIGO</td>";
            echo "<td>NOMBRE</td>";
            echo "<td>MEDIDA</td>";
            ...
            echo "<tr>";
              ...
              foreach($users as $v){
               echo "<td>".$v['codigo']."</td>";
               echo "<td>".$v['nombre']."</td>";
               echo "<td>".$v['nom_medida']."</td>";
               echo "<td>".$v['valor']."</td>";
               echo "<td>".$v['valor_lista']."</td>";
