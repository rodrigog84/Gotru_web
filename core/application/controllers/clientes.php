...
class Clientes extends CI_Controller {
	...
	public function elimina(){
		...
		foreach ($query->result() as $row)	{
			...
			$codigo=$row->id_rubro;
			$id =$row->id;
			$query2 = $this->db->query('SELECT * FROM cod_activ_econ WHERE id='.$codigo.'');
	...
	public function validaRut(){
	   ...
	   if($dv == trim(strtolower($RUT[1]))){
			...
			ven.nombre as nombre_vendedor, g.nombre as giro, cob.nombre as nom_cobrador, rep.nombre as nom_repartidor, tip.descripcion as nom_tiponegocio FROM clientes acc
	...
	public function getAll(){
		...
		if($opcion == "Rut"){
		  ...
		  foreach ($query->result() as $row)
	        ...
	        foreach ($arrayNombre as $nombre) {
	        	$sql_nombre .= "acc.sigla like '%".$nombre."%' and ";
	        }
